<script setup lang="ts">
import 'proj4leaflet';
import L from 'leaflet';

const map = ref<L.Map>();

onMounted(() => {
  map.value = L.map('map', {
    attributionControl: false,
    center: [36.71630859375, 108.63611429929733],
    zoom: 5,
    crs: new L.Proj.CRS('EPSG:4490', '+proj=longlat +ellps=GRS80 +no_defs', {
      resolutions: [
        1.40625, 0.703125, 0.3515625, 0.17578125, 0.087890625, 0.0439453125, 0.02197265625, 0.010986328125, 0.0054931640625, 0.00274658203125,
        0.001373291015625, 6.866455078125e-4, 3.4332275390625e-4, 1.71661376953125e-4, 8.58306884765625e-5, 4.291534423828125e-5,
        2.1457672119140625e-5, 1.0728836059570312e-5, 5.364418029785156e-6, 2.682209064925356e-6, 1.3411045324626732e-6, 0.8411045324626732e-7,
        0.6411045324626732e-7,
      ],
      origin: [-180, 90],
    }),
  });
  L.tileLayer(
    '/EGIS/service/api/egis/base/v1/wmts/1.0.0/kd/default/tiles/c/{z}/{y}/{x}.png?Authorization=Basic ODkzNzgyMGVlNjQ2NDVjNmIxMzM1ZDY4ZmQzMjFkYjM6YmRmMmU4NzE4NDgxNGE5MGExMmU4NzM3NzRiYTI3Zjk=',
    {
      maxZoom: 22,
      minZoom: 4,
    },
  ).addTo(map.value);
});
</script>

<template>
  <div class="h-screen">
    <div class="h-full" id="map"></div>
  </div>
</template>
